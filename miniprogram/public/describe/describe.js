...
Page({
  ...
  hand_in_publish:function(e){    //主函数
    while(1){
  ...
  onLoad: function (options) {
    ...
    wx.request({   //检查用户是否存在
      url: 'https://www.zzumarket.com/check_user.php', //服务器地址
      data: {
      ...
      success: function (res) {
        ...
        if (res.data) {
          console.log(res.data + "查到该用户")
        }
        else {
          wx.request({   //插入新用户
            url: 'https://www.zzumarket.com/insert_user.php', //服务器地址
            data: {
              openid: getApp().globalData.user_openid,
              user_name: getApp().globalData.user_info.nickName,
              user_avaurl: getApp().globalData.user_info.avatarUrl,
              sex: getApp().globalData.user_info.gender,
            },
            header: {
              'content-type': 'application/json'
            },
